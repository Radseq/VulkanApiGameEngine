...
void Terrain::loadAssets( )
{
    ...
    terrainArrayTextureImage =
        terrainArrayTexture.LoadTexture (ResourcePatch::GetInstance( )->GetPatch ("DataPatch") +
                                             "/textures/terrain_texturearray" + texFormatSuffix + ".ktx",
                                         texFormat);
    ...
    samplerInfo.maxLod                            = static_cast<float> (heightMapTextureImage->GetMipLevels( ));
    ...
    heightMapTextureImageSampler.Create (samplerInfo);

    heightMapTextureDescImageInfo.imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal;
    ...
    samplerInfo.maxLod       = static_cast<float> (terrainArrayTextureImage->GetMipLevels( ));
    ...
    terrainArrayTextureImageSampler.Create (samplerInfo);
}
...
void Terrain::generateTerrain( )
{
    ...
    HeightMap heightMap { };
    heightMap.loadHeightMap ("/textures/terrain_heightmap_r16.ktx", PATCH_SIZE);
...
void Terrain::setupDescriptorSets (GraphicCore::DescriptorPool& descPool)
{
    ...
    std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
        ...
        {terrainDescriptorSet, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &heightMapTextureDescImageInfo},
        {terrainDescriptorSet, 2, 0, 1, vk::DescriptorType::eCombinedImageSampler, &terrainArrayTextureDescImageInfo}};
...
void Terrain::createPipeline (const vk::RenderPass& renderPass)
{
    ...
    builder.loadShader (ResourcePatch::GetInstance( )->GetPatch ("DataPatch") + "/shaders/terraintessellation/vert.spv",
