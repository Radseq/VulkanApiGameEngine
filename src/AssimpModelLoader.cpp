 ...
 */
void AssimpModelLoader::loadFromFile (Model* const model, const std::string_view& filename,
                                      ModelCreateInfo* const createInfo)
{
    ...
    for (unsigned int i = 0; i < pScene->mNumMeshes; i++)
    {
        ...
        for (unsigned int j = 0; j < paiMesh->mNumVertices; j++)
        {
            ...
            for (const auto& component : model->GetVertexLayout( ).components)
            {
                switch (component)
                {
        ...
        uint32_t indexBase = GraphicCore::Util::toUint32t (indexBuffer.size( ));
